...
describe( 'resemblesUrl()', () => {
	...
	test( 'should return true for a URL containing query string parameters', () => {
		expect( resemblesUrl( 'https://example.com?foo=bar&baz=qux' ) ).toBe( true );
	} );
	...
	test( 'should return false for a malformed input with consecutive dots', () => {
		expect( resemblesUrl( '..com' ) ).toBe( false );
	} );
	...
	test( 'should return false for a single dot character', () => {
		expect( resemblesUrl( '.' ) ).toBe( false );
	} );

	test( 'should return false for a plain string without any dot separator', () => {
		expect( resemblesUrl( 'example' ) ).toBe( false );
	} );
} );
